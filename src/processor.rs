...
- jerusalem_attack_warning: Non-empty if there is ANY warning or credible report specifically mentioning Jerusalem as a target or at-risk area. Also non-empty if merkaz/center Israel is targeted (missiles to merkaz pass directly over Jerusalem). A confirmed attack on Jerusalem is ALSO a warning — if jerusalem_actual_red_alerts is non-empty, this field must also be non-empty (a square is also a rectangle). Example: "Missiles targeting center over Jerusalem"
