...
CLEARING FIELDS — ONLY when Pikud HaOref gives the ALL CLEAR: Fields must stay non-empty until Pikud HaOref (Home Front Command) explicitly announces BOTH that the event is over AND that it is safe to exit safe spaces (due to falling shrapnel/debris risk). If there is no explicit "you may leave shelters" announcement, keep the fields active. Once Pikud HaOref confirms safe to exit, set fields to "". Past tense + official all-clear = "". The fields reflect CURRENT DANGER ONLY, not recent history.
...
- jerusalem_attack_warning: THIS FIELD TRIGGERS A LOUD ALARM. Non-empty only when there is concrete, credible evidence of an imminent or active threat to Jerusalem — such as confirmed launches toward center Israel, multiple credible sources reporting inbound missiles, or military announcements of an imminent strike on merkaz/Jerusalem. Also non-empty if ANY of the following regions are concretely targeted: Merkaz (center), Yehuda (Judea), or Gush Dan — because all of these flight paths cross directly over Jerusalem. Specifically: missiles targeting Yehuda/Judea/Negev/Otef Aza fly over southern Jerusalem, but Pikud HaOref typically fails to activate Jerusalem sirens for that trajectory. This system must compensate for that gap. Do NOT set non-empty for: vague intelligence rumors, political tension, unverified single-source claims, or speculative "might happen" scenarios. A confirmed attack on Jerusalem is ALSO a warning — if jerusalem_actual_red_alerts is non-empty, this field must also be non-empty (a square is also a rectangle). Example: "Missiles targeting center over Jerusalem"
