...
3. UAVs/DRONES (any origin): Clear fields as soon as the event is reported over ("UAV intercepted", "threat has ended"). No shrapnel wait needed for UAV interceptions.
Once the appropriate condition is met, set fields to "". The fields reflect CURRENT DANGER ONLY, not recent history.
